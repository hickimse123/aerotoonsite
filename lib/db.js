...
const SCHEMA_VERSION = 6; // v6: Sohbete alıntılama (reply_to_id) ve GIF (gif_url) desteği eklendi
...
async function migrateDatabase(db) {
  try {
    // ⚠️ ÖNEMLİ: Bu blok bilinçli olarak fonksiyonun EN BAŞINDA duruyor.
    // migrateDatabase içindeki adımların tamamı TEK bir try/catch'e sarılı
    // (en alttaki catch) — yani aşağıdaki adımlardan biri (veriye özgü bir
    // sebeple) throw ederse, geri kalan HİÇBİR migration çalışmadan
    // fonksiyon sessizce sonlanıyor ve yine de schema_meta versiyon
    // numarası ilerletiliyor (initDb bunu "tamamlandı" sanıyor). Kritik ve
    // bağımsız migrationları en başa alarak, altta bir yerde patlasa bile
    // bunların garanti çalışmasını sağlıyoruz.
    try { await db.exec(`ALTER TABLE chapters ADD COLUMN external_url TEXT DEFAULT NULL`); } catch {}
    try { await db.exec(`ALTER TABLE chapters ADD COLUMN external_note TEXT DEFAULT NULL`); } catch {}
    try {
      await db.exec(`
        INSERT OR IGNORE INTO app_settings (setting_key, setting_value) VALUES ('external_redirect_message', 'Bu bölüm başka bir ekip tarafından hazırlanmıştır. Onları desteklemek için ilgili siteye yönlendiriliyorsunuz. Eğer ekibin sitesi kapandıysa, en az reklam barındıran alternatif bir kaynağa yönlendiriliyorsunuz.');
        INSERT OR IGNORE INTO app_settings (setting_key, setting_value) VALUES ('external_redirect_button_text', 'Siteye Git');
      `);
    } catch {}
    // Sohbet: alıntılama (reply) ve GIF gönderme desteği
    try { await db.exec(`ALTER TABLE chat_messages ADD COLUMN reply_to_id INTEGER DEFAULT NULL`); } catch {}
    try { await db.exec(`ALTER TABLE chat_messages ADD COLUMN gif_url TEXT DEFAULT NULL`); } catch {}
    try {
      await db.exec(`
        INSERT OR IGNORE INTO app_settings (setting_key, setting_value) VALUES ('chat_gif_enabled', '1');
        INSERT OR IGNORE INTO app_settings (setting_key, setting_value) VALUES ('giphy_api_key', '');
      `);
    } catch {}
