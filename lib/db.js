...
async function initializeDatabase(db) {
  ...
  // Feature: Mağaza (Shop) — admin tarafından tanımlanan rozet/çerçeve/unvan,
  // kullanıcılar Yomi Puanı karşılığında satın alıp profillerinde kuşanabilir.
  await db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_shop_items_type ON shop_items(type, is_active);
  ...
  // Migration: equipped cosmetic columns on users (mağaza — kuşanılan öğeler)
  try { await db.exec('ALTER TABLE users ADD COLUMN equipped_frame_id INTEGER DEFAULT NULL'); } catch (e) {}
  try { await db.exec('ALTER TABLE users ADD COLUMN equipped_title_id INTEGER DEFAULT NULL'); } catch (e) {}
  try { await db.exec('ALTER TABLE users ADD COLUMN equipped_badge_id INTEGER DEFAULT NULL'); } catch (e) {}
